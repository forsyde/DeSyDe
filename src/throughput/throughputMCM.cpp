...
ThroughputMCM::ThroughputMCM(Space& home, ViewArray<IntView> p_latency,
...
size_t ThroughputMCM::dispose(Space& home){
...
ThroughputMCM::ThroughputMCM(Space& home, bool share, ThroughputMCM& p): 
...
Propagator* ThroughputMCM::copy(Space& home, bool share){
  return new (home) ThroughputMCM(home, share, *this);
}

//TODO: do this right
PropCost ThroughputMCM::cost(const Space& home, const ModEventDelta& med) const{
  return PropCost::linear(PropCost::HI,next.size());
}

void ThroughputMCM::debug_constructMSAG(){
  //if(printDebug) cout << "\tThroughputMCM::debug_constructMSAG()" << endl;
  //first, figure out how many actors there will be in the MSAG, in order to
  //initialize channel-matrix and actor-vector for the state of MCM
  ...
  */
  ...
  /* 
     for (size_t i=0; i<receivingActors.size(); i++){ 
  ...
  /*
    ...
    for (size_t i=0; i<channelMapping.size(); i+=3){ 
  ...
  if(printDebug){
    /*cout << "initial state for SSE: " << endl;
      ...
      for (size_t j=0; j<n_msagActors; j++){
...
void ThroughputMCM::constructMSAG(){
  if(printDebug) cout << "\tThroughputMCM::constructMSAG()" << endl;
  //first, figure out how many actors there will be in the MSAG, in order to
...
ExecStatus ThroughputMCM::propagate(Space& home, const ModEventDelta&){
  if(printDebug) cout << "\tThroughputMCM::propagate()" << endl;
  ...
  /*  for (size_t i=0; i<2*n_msagActors; i++){
      cout << "-";
      }*/
  /*cout << endl;
    for (size_t i=0; i<n_msagActors; i++){
  ...
  /*  
      ...
      for (size_t i=n_actors; i<max_end.size(); i++){
      cout << i << ": ";
      for (size_t j=0; j<max_end[i].size(); j++){
      cout << "[" << max_start[i][j] << ", " << max_end[i][j] << "] ";
      }
      ...
      for (size_t i=n_actors; i<min_end.size(); i++){
      cout << i << ": ";
      for (size_t j=0; j<min_end[i].size(); j++){
      cout << "[" << min_start[i][j] << ", " << min_end[i][j] << "] ";
      }
 ...
 * tok: |#channels|
 */
void throughputMCM(Space& home, const IntVar latency,
                   const IntVar period,
  ...
  if(ThroughputMCM::post(home, _latency, _period, _iterations, _iterationsCh, 
 ...
 * tok: |#channels|
 */
void throughputMCM(Space& home, const IntVarArgs& latency,
...
int ThroughputMCM::getBlockActor(int ch_id) const{
...
int ThroughputMCM::getSendActor(int ch_id) const{
  auto it = find(channelMapping.begin(), channelMapping.end(), ch_id);
  if(it != channelMapping.end())
...
int ThroughputMCM::getRecActor(int ch_id) const{
  auto it = find(channelMapping.begin(), channelMapping.end(), ch_id);
  if(it != channelMapping.end())
...
int ThroughputMCM::getApp(int msagActor_id) const{
  int id = msagActor_id;
...
void ThroughputMCM::printThroughputGraph(){
  cout << "-------------------------------------------------"<<endl;
...
void ThroughputMCM::printThroughputGraphAsDot(const string &dir) const {
...
void ThroughputMCM::printSchedule(string type, int length){
