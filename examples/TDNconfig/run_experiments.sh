...
for (( i=$first_exp;i<=$last_exp;i++)); do
    ...
    for ((r=1;r<=$total_runs;r++)); do
        ...
        #sudo chrt --rr 99 
        echo ./../../bin/adse --config exp_$i/config.cfg --dse.th_prop MCR --output exp_$i/run$r/ --config-TDN 
        ./../../bin/adse --config exp_$i/config.cfg --dse.th_prop MCR --output exp_$i/run$r/ --config-TDN
        duration=$SECONDS
